...
//Local Business
    ...
    <?php if ($local_business_schema_type == "allSchema_LegalService") { ?>
        ...
        <?php } ?> 
        <?php if (get_option('allSchema_localbusiness_schema_office_timings')) { ?>
                    "openingHours" : "<?php echo get_option('allSchema_localbusiness_schema_office_timings'); ?>",
        <?php } ?> 
        <?php
        $cm = 1;
        $featured_nap_loop = new WP_Query(array('post_type' => 'nap_location', 'posts_per_page' => -1, 'meta_key' => 'is_main_nap_location', 'meta_value' => 'yes'));

        $count_nap_posts_num1 = $featured_nap_loop->found_posts;
        ?>      
        <?php
        if ($featured_nap_loop->have_posts() && !is_home() && !is_front_page()) {
            ?>                  "address" :
                    [
            <?php
            while ($featured_nap_loop->have_posts()) : $featured_nap_loop->the_post();
                ...
                ?>                                 { 
                            ...
                            "addressCountry": "<?php echo esc_attr(get_post_meta(get_the_ID(), 'nap_country', true)) ?>"<?php if (!get_option('allSchema_localbusiness_schema_phone')) { ?> ,
                                 "telephone":"<?php echo esc_attr(get_post_meta(get_the_ID(), 'nap_phone_number', true)) ?>"<?php if (!get_option('nap_office_timings')) { ?> ,
                                   "openingHours" : "<?php echo esc_attr(get_post_meta(get_the_ID(), 'nap_office_timings', true)) ?>"
                    <?php } ?>
                <?php } ?>
                            }   
...
//            $count_nap_posts_num1 = $featured_nap_loop->found_posts;
    ...
    <?php if (get_option('allSchema_enable_client_review_schema') == 'Yes') { ?>
        ...
        <?php $star_rating_title = get_post_meta(get_the_ID(), 'client_review_review_title_field', true); ?>
     <?php if ($star_rating && $star_rating_title) { ?>
            <script type="application/ld+json">
                {
                "@context": "https://schema.org/",
                "@type": "Review",
    ...
    <?php } // end enable review schema   ?>
     <?php if (get_option('allSchema_enable_video_schema') == 'Yes') { ?>
        <?php $video_title = get_post_meta(get_the_ID(), 'video_schema_video_title', true); ?>
        <?php $video_description = get_post_meta(get_the_ID(), 'video_schema_video_description', true); ?>
        <?php $videoImage = get_post_meta(get_the_ID(), 'video_schema_video_image_url', true); ?>
        <?php $uploadDate = get_post_meta(get_the_ID(), 'video_schema_video_date_uploaded', true); ?>
